...
var Renderer = {
  ...
  parseEntities: function(text, entities) {
    ...
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      if (entity.type === 'mention') {
        ...
        if(!(entity.type === "media" && OptionsBackend.get('show_photo_thumbnails'))) {
  ...
  openImageSource: function(entity) {
    return function() {
      if(entity.video_info) {
        openTab(entity.video_info.variants.filter(
          (variant) => variant.content_type == "video\/mp4"
        ).sort(
          (vA, vB) => vB.bitrate - vA.bitrate
        )[0].url);
      } else {
  ...
  renderTweet: function (tweet, useColors, nameAttribute, topLevel) {
    ...
    if(tweet.retweeted_status) {
      ...
      if (tweet.retweeted_status.extended_tweet) {
        ...
        media_entities = tweet.retweeted_status.extended_tweet.entities;
      }
    }
    var content;
