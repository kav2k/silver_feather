...
TwitterLib.prototype = {
  ...
  ajaxRequest: function(url, callback, context, requestParams, httpMethod, useSearchAPI, overriddenTimeout) {
    ...
    if(httpMethod == "GET") {
      requestParams.tweet_mode = "extended";
    }
    var requestUrl;
    if(useSearchAPI) {
  ...
  normalizeTweets: function(tweetsOrTweet) {
    ...
    for(var i = 0, len = tweetsOrTweet.length; i < len; ++i) {
      ...
      if (ti.extended_tweet && ti.extended_tweet.full_text) {
        ti.text = ti.extended_tweet.full_text;
      } else if (ti.full_text) {
        ti.text = ti.full_text;
      }
    }
  },
